...
export default function CreateMoviePage() {
  ...
  useEffect(() => {
    directorsService.getAll().then(setDirectors).catch(() => {})
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.directorId === 0) { setError('Please select a director.'); return }
    ...
    try {
      ...
      setError(message || 'Failed to create movie. Please try again.')
    } finally {
      setLoading(false)
    }
  }
          ...
          <div style={{ maxWidth: 800 }}>
            ...
            {success && (
              <div style={{ padding: '16px', backgroundColor: 'rgba(74,222,128,0.1)', border: '1px solid rgba(74,222,128,0.3)', borderRadius: 6, marginBottom: 24 }}>
                <p style={{ fontFamily: "'Be Vietnam Pro', sans-serif", fontSize: 15, color: '#4ade80' }}>
                  Movie created successfully! Redirecting…
                </p>
              </div>
            )}
            ...
            {error && (
              <div style={{ padding: '16px', backgroundColor: 'rgba(229,9,20,0.1)', border: '1px solid rgba(229,9,20,0.3)', borderRadius: 6, marginBottom: 24 }}>
                <p style={{ fontFamily: "'Be Vietnam Pro', sans-serif", fontSize: 15, color: '#e50914' }}>{error}</p>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
              ...
              <div className="flex gap-4 pt-4">
                ...
                >
                  {loading ? 'Creating…' : success ? 'Created!' : 'Create Movie'}
                </button>
                <button
                  type="button"
                  onClick={() => navigate('/movies')}
